...
func (m *urlMatcher) MatchesURLPattern(requestURL, pattern string) bool {
	...
	// Special case: if pattern URL has empty path and contains wildcards, allow any path (domain-only wildcard
	// matching)
...
func (*urlMatcher) urlPatternToRegex(pattern string) string {
...
func (*urlMatcher) matchesHost(host, pattern string) bool {
	if pattern == "" {
		return true
	}
...
func (*urlMatcher) matchesPath(path, pattern string) bool {
