...
var _ = Describe("DiscoverPlugins", func() {
	...
	createManifestOnlyPlugin := func(name string) {
		...
		manifest := `{
			...
			"author": "Test Author",
			"description": "Test Plugin",
			"website": "https://test.plugins.org/manifest-only",
			"permissions": {}
		}`
		Expect(os.WriteFile(filepath.Join(pluginDir, "manifest.json"), []byte(manifest), 0o600)).To(Succeed())
	}
	...
	createInvalidManifestPlugin := func(name string) {
		...
		Expect(
			os.WriteFile(filepath.Join(pluginDir, "manifest.json"), []byte(invalidManifest), 0o600),
		).To(Succeed())
	}
	...
	createEmptyCapabilitiesPlugin := func(name string) {
		...
		manifest := `{
			...
			"author": "Test Author",
			"description": "Test Plugin",
			"website": "https://test.plugins.org/empty-capabilities",
			"permissions": {}
		}`
		Expect(os.WriteFile(filepath.Join(pluginDir, "manifest.json"), []byte(manifest), 0o600)).To(Succeed())
	}
	...
	BeforeEach(func() {
		tempPluginsDir, _ = os.MkdirTemp("", "plugins-discovery-test-*")
		DeferCleanup(func() {
	...
	Context("Valid plugins", func() {
		It("should discover valid plugins with all required files", func() {
			...
			Expect(testPlugin.Error).ToNot(HaveOccurred())
			Expect(testPlugin.Manifest.Name).To(Equal("Test Plugin"))
			Expect(string(testPlugin.Manifest.Capabilities[0])).To(Equal("IntegrationTest"))

			Expect(anotherPlugin).NotTo(BeNil())
			Expect(anotherPlugin.Error).ToNot(HaveOccurred())
			Expect(anotherPlugin.Manifest.Name).To(Equal("Another Plugin"))
			Expect(string(anotherPlugin.Manifest.Capabilities[0])).To(Equal("LifecycleManagement"))
		})
		...
		It("should handle plugins with same manifest name in different directories", func() {
			...
			Expect(officialPlugin.Error).ToNot(HaveOccurred())
			...
			Expect(customPlugin.Error).ToNot(HaveOccurred())
	...
	Context("Invalid content", func() {
		...
		It("should report error for plugins with empty capabilities", func() {
			...
			Expect(discoveries[0].Error).To(HaveOccurred())
			Expect(
				discoveries[0].Error.Error(),
			).To(ContainSubstring("field capabilities length: must be >= 1"))
		})
	})
	...
	Context("Symlinks", func() {
		It("should discover symlinked plugins correctly", func() {
			...
			realPluginDir, err := os.MkdirTemp("", "real-plugin-*")
			Expect(err).ToNot(HaveOccurred())
			DeferCleanup(func() {
			...
			manifest := `{
				...
				"author": "Test Author",
				"description": "Test Plugin",
				"website": "https://test.plugins.org/symlinked-plugin",
				"permissions": {}
			}`
			Expect(
				os.WriteFile(filepath.Join(realPluginDir, "manifest.json"), []byte(manifest), 0o600),
			).To(Succeed())
			...
			Expect(discoveries[0].Error).ToNot(HaveOccurred())
		...
		It("should handle relative symlinks", func() {
			...
			Expect(os.MkdirAll(realPluginDir, 0o755)).To(Succeed())
			DeferCleanup(func() {
				_ = os.RemoveAll(realPluginDir)
			})
			...
			manifest := `{
				...
				"author": "Test Author",
				"description": "Test Plugin",
				"website": "https://test.plugins.org/relative-symlinked-plugin",
				"permissions": {}
			}`
			Expect(
				os.WriteFile(filepath.Join(realPluginDir, "manifest.json"), []byte(manifest), 0o600),
		...
		It("should report error for symlinks pointing to files", func() {
			...
			Expect(os.WriteFile(regularFile, []byte("content"), 0o600)).To(Succeed())
	...
	Context("Directory filtering", func() {
		...
		It("should ignore regular files", func() {
			...
			Expect(
				os.WriteFile(
					filepath.Join(tempPluginsDir, "regular-file.txt"),
					[]byte("content"),
					0o600,
				),
			).To(Succeed())
