...
var _ = Describe("Plugin Manifest", func() {
	...
	It("should load and parse a valid manifest", func() {
		...
		manifestContent := []byte(`{
			...
			"capabilities": ["IntegrationTest", "LifecycleManagement"],
			"permissions": {
				"http": {
		...
		Expect(manifest.Capabilities[0]).To(Equal(schema.PluginManifestCapabilitiesElemIntegrationTest))
		Expect(manifest.Capabilities[1]).To(Equal(schema.PluginManifestCapabilitiesElemLifecycleManagement))
	...
	It("should fail with JSON parse error for invalid JSON", func() {
		...
		Expect(
			os.WriteFile(filepath.Join(pluginDir, "manifest.json"), []byte(invalidJSON), 0o600),
		).To(Succeed())
