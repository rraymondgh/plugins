...
func (s WebSocketHostFunctions) Connect(
	ctx context.Context,
...
func (s WebSocketHostFunctions) SendText(
	ctx context.Context,
	req *websocket.SendTextRequest,
) (*websocket.SendTextResponse, error) {
	return s.ws.sendText(ctx, s.pluginID, req)
}

func (s WebSocketHostFunctions) SendBinary(
	ctx context.Context,
	req *websocket.SendBinaryRequest,
) (*websocket.SendBinaryResponse, error) {
	return s.ws.sendBinary(ctx, s.pluginID, req)
}

func (s WebSocketHostFunctions) Close(
	ctx context.Context,
	req *websocket.CloseRequest,
) (*websocket.CloseResponse, error) {
...
func (s *websocketService) connect(
	...
	if permissions != nil {
		if err := permissions.IsConnectionAllowed(req.GetUrl()); err != nil {
			zap.L().Warn("WebSocket connection blocked by permissions",
				zap.String("plugin", pluginID), zap.String("url", req.GetUrl()), zap.Error(err))

			return &websocket.ConnectResponse{
				Error: "Connection blocked by plugin permissions: " + err.Error(),
			}, nil
		}
	}
	...
	conn, resp, err := dialer.DialContext(ctx, req.GetUrl(), header)
	if err != nil {
	...
	zap.L().Debug("WebSocket connection established",
		zap.String("plugin", pluginID),
		zap.String("connectionID", connectionID), zap.String("url", req.GetUrl()))
...
func (s *websocketService) sendText(
	_ context.Context,
	pluginID string,
	req *websocket.SendTextRequest,
) (*websocket.SendTextResponse, error) {
	if err := s.writeMessage(pluginID, req.GetConnectionId(),
		gorillaws.TextMessage, []byte(req.GetMessage())); err != nil {
...
func (s *websocketService) sendBinary(
	_ context.Context,
	pluginID string,
	req *websocket.SendBinaryRequest,
) (*websocket.SendBinaryResponse, error) {
	if err := s.writeMessage(pluginID, req.GetConnectionId(), gorillaws.BinaryMessage, req.GetData()); err != nil {
...
func (s *websocketService) close(
	...
	internal := internalConnectionID(pluginID, req.GetConnectionId())
	...
	err := conn.Conn.WriteControl(
		...
		gorillaws.FormatCloseMessage(int(req.GetCode()), req.GetReason()),
		time.Now().Add(time.Second),
	)
	if err != nil {
		zap.L().Error("Error sending close message", zap.String("plugin", pluginID), zap.Error(err))
	}
	...
	zap.L().Debug("WebSocket connection closed",
		zap.String("plugin", pluginID), zap.String("connectionID", req.GetConnectionId()))
...
func (s *websocketService) handleMessages(internalID string, conn *WebSocketConnection) {
	...
	if err != nil {
		zap.L().Error("Invalid internal connection ID", zap.String("id", internalID), zap.Error(err))
		return
	}
	...
	defer func() {
		...
		zap.L().Debug("WebSocket message handler stopped",
			zap.String("plugin", conn.PluginName), zap.String("connectionID", connectionID))
	}()
...
func (s *websocketService) executeCallback(
	...
	if p == nil {
		zap.L().Error("Plugin not found for WebSocket callback")
		return
	}
	...
	_, _ = CallMethod(ctx, p, methodName, func(inst api.WebSocketCallback) (struct{}, error) {
		...
		if err := fn(ctx, inst); err != nil {
			zap.L().
				Error("Error executing WebSocket callback", zap.Duration("elapsed", time.Since(start)), zap.Error(err))
			return struct{}{}, fmt.Errorf("error executing WebSocket callback: %w", err)
		}

		zap.L().Debug("WebSocket callback executed", zap.Duration("elapsed", time.Since(start)))
...
func (s *websocketService) notifyTextCallback(
	ctx context.Context,
	connectionID string,
...
func (s *websocketService) notifyBinaryCallback(
	ctx context.Context,
	connectionID string,
	conn *WebSocketConnection,
	data []byte,
...
func (s *websocketService) notifyErrorCallback(
	ctx context.Context,
	connectionID string,
	conn *WebSocketConnection,
	errorMsg string,
...
func (s *websocketService) notifyCloseCallback(
	ctx context.Context,
	connectionID string,
	conn *WebSocketConnection,
	code int,
