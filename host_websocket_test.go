...
var _ = Describe("WebSocket Host Service", func() {
	...
	BeforeEach(func() {
		...
		manager = createManager(metrics.NewNoopInstance(), TestConfig())
	...
	Describe("WebSocket operations", func() {
		...
		It("connects to a WebSocket server", func() {
			...
			Expect(resp.GetConnectionId()).ToNot(BeEmpty())
		...
		It("returns error when attempting to use non-existent connection", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(sendResp.GetError()).To(ContainSubstring("connection not found"))
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(closeResp.GetError()).To(ContainSubstring("connection not found"))
		})
	})
})
