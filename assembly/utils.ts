...
function fromMemory(ptr: usize, size: u32): Uint8Array {
...
function returnHost(data: Uint8Array): u64 {
...
export class Plugin {
  static request<TRequest>(
    ...
    return Protobuf.decode<TRequest>(fromMemory(ptr, size), decoder);
  }
  ...
  static response<TResponse>(
    ...
    return returnHost(Protobuf.encode<TResponse>(message, encoder));
  }
  ...
  static host<TRequest, TResponse>(
    ...
    if (error) {
      return new returnHelper<TResponse>(
        error,
        changetype<usize>(fromMemory(ptr, size))
      );
    } else {
      return new returnHelper<TResponse>(
        error,
