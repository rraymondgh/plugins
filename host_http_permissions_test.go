...
var _ = Describe("HTTP Permissions", func() {
	...
	Describe("IsRequestAllowed", func() {
				...
				perms = &httpPermissions{
					...
					AllowedUrls: map[string][]string{
						"https://api.example.com": {
							"GET",
							"POST",
						}, // Both uppercase for consistency
					},
					matcher: newURLMatcher(),
				}
			})
		...
		Context("with complex URL patterns and HTTP methods", func() {
			...
			DescribeTable(
				"complex pattern and method combinations",
				func(url, method string, shouldSucceed bool) {
					err := perms.IsRequestAllowed(url, method)
					if shouldSucceed {
						Expect(err).ToNot(HaveOccurred())
				...
				Entry(
					"POST to specific users endpoint",
					"https://api.example.com/v1/users",
					"POST",
					true,
				),
				Entry(
					"PUT to specific users endpoint",
					"https://api.example.com/v1/users",
					"PUT",
					true,
				),
				Entry(
					"DELETE to specific users endpoint",
					"https://api.example.com/v1/users",
					"DELETE",
					false,
				),
				...
				Entry(
					"GET to public path on subdomain",
					"https://cdn.example.com/public/assets",
					"GET",
					true,
				),
				Entry(
					"HEAD to public path on subdomain",
					"https://static.example.com/public/files",
					"HEAD",
					true,
				),
				Entry(
					"POST to public path on subdomain",
					"https://api.example.com/public/upload",
					"POST",
					false,
				),
