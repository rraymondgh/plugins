...
func (p *IntegrationTestPlugin) Load(ctx context.Context, pluginPath string) (integrationTest, error) {
	...
	configtest := module.ExportedFunction("integration_test_config_test")
	if configtest == nil {
		return nil, errors.New("integration_test_config_test is not exported")
	}
	httptest := module.ExportedFunction("integration_test_http_test")
	if httptest == nil {
		return nil, errors.New("integration_test_http_test is not exported")
	}
...
type integrationTestPlugin struct {
	...
	malloc     api.Function
	free       api.Function
	configtest api.Function
	httptest   api.Function
}

func (p *integrationTestPlugin) ConfigTest(ctx context.Context, request *SimpleRequest) (*SimpleResponse, error) {
	...
	ptrSize, err := p.configtest.Call(ctx, dataPtr, dataSize)
	if err != nil {
		return nil, err
	}
...
func (p *integrationTestPlugin) HTTPTest(ctx context.Context, request *HttpRequest) (*SimpleResponse, error) {
	data, err := request.MarshalVT()
	if err != nil {
		return nil, err
	}
	...
	ptrSize, err := p.httptest.Call(ctx, dataPtr, dataSize)
	if err != nil {
		return nil, err
	}
